...
@Slf4j
@Component
public class DDnetGroupMessageListener {
     ...
     */
    @OnGroup
    @Filter(value = "ddnet", matchType = MatchType.CONTAINS, atBot = true)
    ...
    public void queryPoints(GroupMsg msg, MsgSender sender, SimbotContext context){
        if(!isPoints){
            sender.SENDER.sendGroupMsg(msg, "dd查分功能，当前处于关闭状态，请联系管理员开启。");
            sender.SENDER.sendGroupMsg(msg, "❤️管理员列表❤️\n" +
                    admin.toString());
            return;
        }
        try{
            ...
            LogUtil.sendGroupListenLog(msg, result.toString());
            sender.SENDER.sendGroupMsg(msg, result.toString());
        }catch (Exception e){
            e.printStackTrace();
            sender.SENDER.sendGroupMsg(msg.getGroupInfo(), "获取失败！可能是服务器正在维护~~。请联系管理员修复哦❤️\n" +
                    "管理员qq：" + master);
        }
    }

    @OnGroup
    @Filter(value = "关闭dd查分", matchType = MatchType.EQUALS)
    public void shutDDQueryPoints(GroupMsg msg, MsgSender sender, SimbotContext context) {
        if(!isPoints){
            sender.SENDER.sendGroupMsg(msg, "dd查分功能，已经处于关闭状态，无需修改");
            return;
        }
        try{
            ...
            if(!b){
                ...
                sender.SENDER.sendGroupMsg(msg, "✅关闭成功");
            }else{
                sender.SENDER.sendGroupMsg(msg, "对不起，您不是管理员，无法使用此指令！");
            }
        }catch (Exception e){
            e.printStackTrace();
            sender.SENDER.sendGroupMsg(msg, "❌关闭失败！程序异常");
        }
    }


    @OnGroup
    @Filter(value = "开启dd查分", matchType = MatchType.EQUALS)
    public void openDDQueryPoints(GroupMsg msg, MsgSender sender, SimbotContext context) {
        if(isPoints){
            sender.SENDER.sendGroupMsg(msg, "dd查分功能，已经处于开启状态，无需修改");
            return;
        }
        try{
            boolean b = admin.stream().anyMatch(e -> {
            ...
            if(!b){
                ...
                sender.SENDER.sendGroupMsg(msg, "✅开启成功");
            }else{
                sender.SENDER.sendGroupMsg(msg, "对不起，您不是管理员，无法使用此指令！");
            }
        }catch (Exception e){
            e.printStackTrace();
            sender.SENDER.sendGroupMsg(msg, "❌开启失败！程序异常");
        }
    }
}
